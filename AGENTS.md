...
- **Never commit without being asked.** Do not run `git commit` (or `git push`) unless the user explicitly asks for it in that message. Leave finished work in the working tree and say what is ready; the user decides when it lands.
